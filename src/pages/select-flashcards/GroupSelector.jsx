...
export const GroupSelector = ({ name, group, selectedSets, onChange }) => {
  const sets = sortBy(group.sets, ['title']) || [];

  // Determine if all sets are selected
  const allSelected =
    sets.length > 0 && sets.every((s) => selectedSets.includes(s.id));
  const someSelected =
    sets.some((s) => selectedSets.includes(s.key)) && !allSelected;
